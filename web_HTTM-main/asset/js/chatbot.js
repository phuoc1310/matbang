// ================== CHATBOT AI MODULE ==================
// Quản lý chatbot hỗ trợ người dùng với tích hợp AI

// ================== CONFIG ==================
const CHATBOT_CONFIG = {
    // Gemini API Configuration
    USE_GEMINI: true, // Bật/tắt Gemini API
    GEMINI_API_KEY: 'AIzaSyBs3GXepe-whLh2-gInc0me-zA2iG1JHKw', // API Key của Gemini (sẽ được load từ localStorage)
    GEMINI_API_URL: 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent',
    // Fallback: n8n webhook hoặc API khác
    AI_API_URL: 'https://n8n.your-domain/webhook/chatbot-ai', // Placeholder
...
function toggleChatbot(e) {
    ...
    console.log('💬 Chatbot toggled:', !isOpen ? 'OPEN' : 'CLOSE');
...
async function getAIResponse(userMessage) {
    ...
    if (CHATBOT_CONFIG.USE_GEMINI && CHATBOT_CONFIG.GEMINI_API_KEY) {
        try {
            ...
            console.warn('Gemini API lỗi, thử fallback:', error);
        }
    }
    
    // Ưu tiên 2: Custom API (n8n webhook hoặc API khác)
    if (!CHATBOT_CONFIG.FALLBACK_MODE && CHATBOT_CONFIG.AI_API_URL !== 'https://n8n.your-domain/webhook/chatbot-ai') {
        try {
            const response = await fetch(CHATBOT_CONFIG.AI_API_URL, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                return data.response || data.answer || 'Tôi đã nhận được câu hỏi của bạn.';
            }
        } catch (error) {
            console.warn('AI API không khả dụng, sử dụng fallback mode');
        }
    }
    
    // Fallback: Trả lời dựa trên keywords
...
async function callGeminiAPI(userMessage) {
    ...
    const systemPrompt = `Bạn là trợ lý AI thân thiện của SpaceRent - nền tảng kết nối không gian kinh doanh hàng đầu Việt Nam.
...
Hãy trả lời câu hỏi của người dùng một cách tự nhiên và hữu ích.`;
    ...
    try {
        const response = await fetch(
            `${CHATBOT_CONFIG.GEMINI_API_URL}?key=${CHATBOT_CONFIG.GEMINI_API_KEY}`,
            {
                method: 'POST',
                headers: {
